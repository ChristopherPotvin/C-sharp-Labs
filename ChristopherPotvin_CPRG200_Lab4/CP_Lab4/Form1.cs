...
/// Author: Chris Potvin
/// Date: 1/15/2019
/// Details: This is a C# windows form application where two tables have various data types (Orders and Order Details) have been imported into my project using an entity
/// framework. Below is three list objects calling each class (Order and Order_Detail). Both data grid views are populated using the primary key 
/// of order ID from the orders table to switch on the combo box. The sum is then calculated using a method and formula to get the order total based
/// on the ID number. Methods are using LINQ statements to execute the queries from each respective table. Lastly there are another couple of methods created
/// to check validation of the nulls if that is the respective datatype in the db and to ensure that the shipped date is not before the order date
/// and after the required date. 
///
...
namespace CP_Lab4
{
    public partial class Form1 : Form
    {
        ...
        // defining the objects for order table and order details
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            var orderDetailIDs = (from orderDetails in entities.Order_Details
                                  ...
                                  select orderDetails.OrderID).ToList(); // ToList executs the query for the second data view, order_details
        ...
        private void orderIDComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            CheckNull(); // checking to see if the data type from the various dates are set to null
